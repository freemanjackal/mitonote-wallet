...
private:
  ...
  const QDir m_mitocoinDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfMitocoinBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _mitocoin) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo mitocoinBlockchainInfo() const;
